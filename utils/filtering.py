"""
Functions for data filtering tasks. Source: https://raw.githubusercontent.com/paulvangentcom/heartrate_analysis_python/master/heartpy/filtering.py
"""
import logging
        ...
        for filterstring in filters:
            if filterstring in ['BWR', 'BLA']:
                ...
                try:
                    ...
                    logging.warning('Could not process filter "{}". Unknown format. Format should be: "HP/LP/AC DOUBLE Hz" (whithout blanks).'.format(filterstring))
...
def filter_signal(data, cutoff, sample_rate, order=2, filtertype='lowpass', return_top=False):
    ...
    if filtertype.lower() == 'lowpass':
        ...
        assert type(
            cutoff) == tuple or list or np.array, 'if bandpass filter is specified, cutoff needs to be array or tuple specifying lower and upper bound: [lower, upper].'
